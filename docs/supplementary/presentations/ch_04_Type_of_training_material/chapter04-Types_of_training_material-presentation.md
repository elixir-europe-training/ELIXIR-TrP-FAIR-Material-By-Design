...
![](../../../assets/images/FAIRprinciples-R.svg)<!-- style = "width: 150px; padding:15px;"-->
...
|---|---|---|---|---|
| Presentations | PPTX (PowerPoint) | Proprietary/Closed | Provide original file; Export to ODP; convert to PDF/A for archiving; | use open alternatives like LibreOffice Impress; consider reveal.js or similar HTML5 presentation frameworks for maximum portability. |
| Presentations | ODP (OpenDocument Presentation) | Open Standard | Use ODP as primary format with LibreOffice Impress or similar tools;  include media resources;  | ensure compatibility testing across different viewers; document any version-specific features used.|
| Presentations | HTML5 (Marp, reveal.js, Slidev) | Open/Web Standard | Use HTML5-based presentation frameworks; store presentations in version control (Git); use Markdown for content where possible;  | embed resources or link to stable URLs; provide offline viewing capability.|
| Textual Documents | DOCX (Word) | Proprietary/Closed | Provide original; Convert to ODT; export to PDF/A for archiving; | use styles consistently for structure; avoid proprietary features; consider pandoc for conversion to multiple formats. |
| Textual Documents | ODT (OpenDocument Text) | Open Standard | Use ODT as primary format; ensure style-based formatting for structure; export to PDF/A for long-term preservation. | validate with multiple ODF-compliant applications; embed or link fonts;|
| Textual Documents | Markdown/LaTeX/Plain Text | Open/Plain Text | Prefer Markdown for simple documents;  store in version control (Git); | LaTeX for complex technical content; use pandoc for multi-format conversion; include metadata headers; ensure UTF-8 encoding; maintain human readability.|
...
| Scripts/Software | Shell Scripts/Configuration (.sh, .yml, .json) | Open/Plain Text | Use plain text formats (YAML, JSON, TOML); include comments explaining purpose; specify interpreter/tool versions; store in version control (Git); document environment requirements;  | validate syntax; consider using standardized configuration schemas; ensure cross-platform compatibility where possible. |
| Datasets | any formats possible | Open | create descriptive comments about the datasets (metadata); store in version control if possible;  otherwise store in public storage buckets. | store in Zenodo (if possible) for DOI; |
